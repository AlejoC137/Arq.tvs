...
import {
    ...
    RefreshCw // <-- MODIFICACIÓN: Icono añadido
} from 'lucide-react';
...
const StaffTaskModal = () => {
    ...
    const updateCell = (rowId, fieldsToUpdate) =>
        dispatch(updateTask(rowId, fieldsToUpdate))
            .then(() => setStaffTasks(p => p.map(i => i.id === rowId ? { ...i, ...fieldsToUpdate } : i)));
    ...
    const handleDuplicateTasks = () =>
        Promise.all(staffTasks.filter(t => selectedRows.has(t.id)).map(t => {
    ...
    const fetchData = useCallback(async () => {
        setLoading(true);
        try {
            const [tareasAction, staffAction, stagesAction, entregablesAction, projectsAction] = await Promise.all([
            ...
            if (staffAction?.payload) {
                setAllStaff(staffAction.payload);
                const member = staffAction.payload.find(s => s.id === staffId);
                if (member) setStaffMember(member);
            }
            ...
            if (projectsAction?.payload) setProjects(projectsAction.payload);
            if (tareasAction?.payload) setStaffTasks(tareasAction.payload.filter(t => t.staff_id === staffId));
        } catch (err) {
            setError(err.message);
        } finally {
            setLoading(false);
        }
    }, [dispatch, staffId, isFormOpen]);

    useEffect(() => { fetchData(); }, [fetchData]);

    const staffProgress = useMemo(() => {
        if (!staffTasks || staffTasks.length === 0) return 0;
        return Math.round(
            staffTasks.reduce((sum, task) => sum + (Number(task.Progress) || 0), 0) / staffTasks.length
        );
    }, [staffTasks, isFormOpen]);
    ...
    const filteredAndSortedTasks = useMemo(() => {
        ...
        return items;
    }, [staffTasks, sortConfig, searchTerm]);

    if (loading && !staffMember) return <div className="p-8 text-center">Cargando...</div>;
    if (error) return <div className="p-8 text-center text-red-500">Error: {error}</div>;
    if (!staffMember) return <div className="p-8 text-center">Miembro del staff no encontrado.</div>;
    ...
    // ================== MODIFICACIÓN ==================
    // La recarga automática ya la maneja el 'useEffect' 
    // que depende de 'isFormOpen'.
    // Simplemente quitamos la llamada a .then(fetchTasks)
    // ==================================================
    ...
    const EditableCell = ({ rowId, field, value, type = 'text', options = [], onExitEditing = () => { } }) => {
        ...
        const displayValue = (field, val) => {
            switch (field) {
                case 'project_id': return projects.find(p => p.id === val)?.name || val || '-';
                case 'staff_id': return allStaff.find(s => s.id === val)?.name || val || '-';
        ...
        const printTask = () => {
            ...
            container.innerHTML = `
                <div class="hdr">
                    ...
                    <div class="chips">
                        <span class="chip">${proyectoNombre}</span>
            ...
            <div ref={taskRef} className={`relative ${isSelected ? 'bg-blue-50' : 'bg-white'}`} data-print-block="true">
                ...
                <div className="flex items-center w-full pl-6 pr-4 py-2">
                    ...
                    <div className="flex items-center gap-3 md:gap-6 mx-4 md:mx-6 text-sm text-gray-600 flex-shrink-0">
                        <div className="flex items-center gap-2" title="Proyecto">
                            <Tag size={16} className="text-gray-400" />
                            <span>{projects.find(p => p.id === task.project_id)?.name || 'Sin proyecto'}</span>
                        </div>
    ...
    return (
        <div className="h-screen bg-gray-50 flex flex-col p-4 md:p-8 gap-6">
            <FormTask
                ...
                estados={ESTADOS}
                staffIdPrefill={staffId}
            />

            <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200 flex-shrink-0">
                <h1 className="text-3xl font-bold text-gray-800 mb-4">Tareas de {staffMember.name}</h1>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-sm">
                    <div className="flex items-start gap-3">
                        <User className="w-5 h-5 text-gray-400 mt-0.5" />
                        <div>
                            <p className="font-semibold text-gray-600">Rol</p>
                            <p className="text-gray-800">{staffMember.role_description || 'No especificado'}</p>
                        </div>
                    </div>
                    <div className="flex items-start gap-3">
                        <Tag className="w-5 h-5 text-gray-400 mt-0.5" />
                        <div>
                            <p className="font-semibold text-gray-600">Total de Tareas</p>
                            <p className="text-gray-800">{staffTasks.length} tarea{staffTasks.length !== 1 ? 's' : ''}</p>
                        </div>
                    </div>
                    <div className="flex items-start gap-3">
                        <Tag className="w-5 h-5 text-gray-400 mt-0.5" />
                ...
                <div className="mt-6">
                    <div className="flex justify-between items-center mb-1">
                        ...
                        <span className="text-sm font-bold text-blue-600">{staffProgress}%</span>
                    </div>
                    <div className="w-full bg-gray-200 rounded-full h-2.5">
                        <div className="bg-blue-600 h-2.5 rounded-full" style={{ width: `${staffProgress}%` }}></div>
                    </div>
                </div>
            </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 flex flex-col flex-grow min-h-0">
                <div className="p-4 border-b border-gray-200 flex flex-wrap items-center justify-between gap-4 flex-shrink-0">
                    <div className="flex items-center gap-4">
                        ...
                        {/* ================== MODIFICACIÓN ================== */}
                        {/* Botón de recarga manual añadido        */}
                        {/* ================================================== */}
                        <button
                            onClick={fetchData} // Llama a la función principal de carga
