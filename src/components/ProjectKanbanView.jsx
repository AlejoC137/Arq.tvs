                    ...
                    {tasks.map(task => (
                      <div key={task.id} className={`bg-white rounded-lg shadow-sm p-3 mb-3 hover:shadow-lg transition-shadow ${getPriorityColor(task.Priority)}`}>
                        ...
                        <div className="flex justify-between items-start">
                          <p className="font-semibold text-sm text-gray-800 break-words mb-2">{task.tema}</p>
                          <button onClick={() => handleDeleteTask(task.id, task.tema)} className="text-gray-400 hover:text-red-500 p-1 rounded-full flex-shrink-0"><Trash2 size={14} /></button>
                        </div>
